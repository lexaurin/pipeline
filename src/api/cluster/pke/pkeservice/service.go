...
func (s service) RegisterNodeStatus(ctx context.Context, clusterIdentifier cluster.Identifier, nodeStatus NodeStatus) (resp RegisterNodeStatusResponse, err error) {
	...
	if proc.Id == "" {
		proc.Id = uuid.Must(uuid.NewV4()).String()
		...
		if err != nil {
			return resp, errors.WrapIf(err, "failed to get existing process")
		}
		...
		if proc.OrgId != existing.OrgId || proc.ResourceId != existing.ResourceId || proc.ResourceType != existing.ResourceType || proc.Type != existing.Type {
			return resp, errors.New("invalid process ID")
		}
	}
	...
	event, err = s.processes.LogProcessEvent(ctx, event)
	if err != nil {
		return resp, err
	}
	...
	if nodeStatus.Finished && nodeStatus.Failure {
		signalValue := errors.Errorf("%s failed: %s", nodeStatus.Name, nodeStatus.Message)
		for _, cp := range clusterProcesses {
			// send signals with best effort
			_ = s.processes.SignalProcess(ctx, cp.Id, "node-bootstrapped", signalValue)
			_ = s.processes.SignalProcess(ctx, cp.Id, "teszt", "helo")
		}
	}
